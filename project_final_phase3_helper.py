...
from project_final_helper import search_frame_by_image
...
def phase_3_mega_function(prompt: str):
    PROMPT = prompt
    ...
    def evaluate_image_with_questions(img_path, questions):
        print(f"\n🔍 Evaluating: {img_path}")
        image = Image.open(img_path)
        image.show()  # Optional: show image

        for item in questions:
            question = item['question']
            majority, responses = get_majority_answer(question, img_path)
            print(f"Q: {question}")
            print(f"→ Responses: {responses}")
            print(f"→ Inferred: {majority}\n")

    # Run evaluation for each image returned by CLIP
    search_results = search_frame_by_image("./converted_image.jpg", top_k=5)

    for hit in search_results:
        frame_path = f"keyframes/{hit['video_id']}/{hit['frame_path']}"
        evaluate_image_with_questions(frame_path, verified_questions)
